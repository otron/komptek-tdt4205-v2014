...
That's the one I used to answer this.

% man I am pretty lazy
\newcommand{\blockn}{B1}
\subsubsection{\blockn}
$gen(\text{\blockn}) = \{d_1,~d_2,~d_3\}$

$kill(\text{\blockn}) = \{d_5, ~d_{10}, ~d_6, ~d_8, ~d_{11}, ~d_{12}\}$

$\IN{\blockn} = $

$\OUT{\blockn} = $

\renewcommand{\blockn}{B2}
\subsubsection{\blockn}
$gen(\text{\blockn}) = \{d_4,~d_5\}$

$kill(\text{\blockn}) = \{d_1,~ d_{10}\}$

$\IN{\blockn} = $

$\OUT{\blockn} = $

\renewcommand{\blockn}{B3}
\subsubsection{\blockn}
$gen(\text{\blockn}) = \{d_6,~ d_7\}$

$kill(\text{\blockn}) = \{d_2,~ d_8\}$

$\IN{\blockn} = $

$\OUT{\blockn} = $

\renewcommand{\blockn}{B4}
\subsubsection{\blockn}
$gen(\text{\blockn}) = \{d_8,~ d_9\}$
...
\renewcommand{\blockn}{B5}
\subsubsection{\blockn}
$gen(\text{\blockn}) = \{d_{10},~ d_{12}\}$

$kill(\text{\blockn}) = \{d_{10},~ d_{12}\}$

$\IN{\blockn} = $

$\OUT{\blockn} = $

\renewcommand{\blockn}{EXIT}
\subsubsection{\blockn}
$gen(\text{\blockn}) = \emptyset$

$kill(\text{\blockn}) = \emptyset$
