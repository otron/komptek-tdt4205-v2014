...
Well so it's just about the symbol tables then?
Because I don't think the point is that we should be getting multiple declaration issues or type errors or whatever when using the single symbol table.
It's the language that supports blocks and scopes, so the single-ST implementation should have to be able to handle it, right?
